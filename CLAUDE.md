@oomfware/forms is a form validation middleware for @oomfware/fetch-router, built with TypeScript.

the source code is organized under `src/`:

- `lib/form.ts`: core `form()` function, types, and form processing logic
- `lib/form-utils.ts`: utilities for form data parsing, field proxies, and issue handling
- `lib/middleware.ts`: the `forms()` router middleware
- `lib/errors.ts`: validation error helpers (`invalid()`, `ValidationError`)
